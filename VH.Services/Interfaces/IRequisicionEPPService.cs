...
namespace VH.Services.Interfaces
{
    public interface IRequisicionEPPService
    {
        ...
        Task<(bool Success, string? Error)> EntregarAsync(int id, string userId, string firmaDigital, string? fotoEvidencia, string? observaciones, List<(int IdDetalle, int IdCompraDetalle, decimal CantidadEntregada)> detalles);
